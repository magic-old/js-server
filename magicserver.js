...
var _magiclogger = require('magiclogger');

var _magiclogger2 = _interopRequireDefault(_magiclogger);
...
var MagicServer = (function () {
  function MagicServer() {
    ...
    (0, _magiclogger2['default'])('collecting files in ' + dir);

    var globbed = (0, _glob.sync)(dir, { nodir: true });
    (0, _magiclogger2['default'])('found ' + globbed.length + ' files');
  ...
  _createClass(MagicServer, [{
    ...
    value: function serve(files) {
      ...
      (0, _magiclogger2['default'])('start server');
      ...
      (0, _http.createServer)(function (req, res) {
        ...
        _magiclogger2['default'].request(ip, url, startTime, endTime);
      })

      // Listen to port set in config
      .listen(port, function () {
        _magiclogger2['default'].success('server listening to localhost:' + port);
      });
    }
  }, {
    key: 'error404',
    value: function error404(url, res) {
      _magiclogger2['default'].error('could not find file with url: ' + url);
